...
# Full model object, i.e. what is saved as output from fine-tuning 
# Case: Fine-tuning with GmP, convert with exp-ft-C-convert-Long-GmP-back-to-weight.py first, use converted model here:
finetunedclip = 'I:/LongCLIPmodelGMP/ft-checkpoints/converted_longclip_ft_10.pt'
_, preprocess = longclip.load("checkpoints/longclip-L.pt", device=device)
model = torch.load(finetunedclip).to(device)
...
print(Fore.GREEN + f"Getting CLIP opinion about image {args.image_path}\n" + Fore.RESET)
