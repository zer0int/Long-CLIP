...
# My results were actually archived after 10 EPOCHS, and I stopped training / CTRL+C. 
# But as scheduler depends on Epochs, I am leaving this as-is for reproduction.
EPOCHS = 20
learning_rate = 3e-7 # only used if custom param_groups NOT used in optimizer (NOT recommended for GmP fine-tune).
max_learning_rate = 1e-7 # for scheduler.
batch_size = 34 # This is what I used and what comfortably fit


# 1. Download SPRIGHT-COCO from: https://huggingface.co/datasets/SPRIGHT-T2I/spright_coco
# 2. Square center-crop the images (because CLIP's input dimension is 224x224, i.e. square)

# 3. Use the following to reproduce my results, I provide the .json in folder "SPRIGHT-COCO":
dataset1 = ImageTextDataset("SPRIGHT-COCO/data-square", "SPRIGHT-COCO/coco-sprite-train-0_9.json", transform=preprocess)
...
val_dataset = ImageTextDataset("SPRIGHT-COCO/data-square", "SPRIGHT-COCO/coco-sprite-val-10_11.json", transform=preprocess)
...
# These seem insanely low, but it's what I used to get good results / stop gradients from exploding to 'inf' after epoch 0:
param_groups = [
    {'params': visual_parameters, 'lr': 8e-8},
    {'params': transformer_parameters, 'lr': 8e-8},
    {'params': model.text_projection, 'lr': 8e-8},
    {'params': model.token_embedding.parameters(), 'lr': 8e-8},
    {'params': [model.positional_embedding, model.positional_embedding_res], 'lr': 1e-8},
    {'params': [model.visual.class_embedding, model.visual.positional_embedding, model.visual.proj], 'lr': 1e-8}, # Mess with these muchly ...
    {'params': [model.visual.ln_pre.weight, model.visual.ln_pre.bias, model.visual.ln_post.weight, model.visual.ln_post.bias], 'lr': 1e-8}, #...
    {'params': [model.ln_final.weight, model.ln_final.bias, model.visual.conv1.weight], 'lr': 1e-8}  # and gradients *will* explode.
]
